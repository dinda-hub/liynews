...
$kat = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT * FROM kategori WHERE id_kategori=$id"));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama)) {
        ...
        $cek = mysqli_query($koneksi, "SELECT id_kategori FROM kategori WHERE nama_kategori='$nama' AND id_kategori != $id");
        if (mysqli_num_rows($cek) > 0) {
            $error = "Nama kategori sudah digunakan.";
        } else {
            mysqli_query($koneksi, "UPDATE kategori SET nama_kategori='$nama', deskripsi='$deskripsi' WHERE id_kategori=$id");
            header("Location: kategori.php?msg=edit"); exit;
        }
    }
}
?>
...
<head>
  ...
  <title>Edit Kategori — LiyNews Admin</title>
...
.top-crumb a{color:rgba(255,255,255,.5);transition:.15s}
.top-crumb a:hover{color:rgba(255,255,255,.85)}
...
.btn-back{display:inline-flex;align-items:center;gap:8px;padding:10px 22px;background:none;color:var(--muted);border:1.5px solid var(--border);border-radius:var(--r);font-family:var(--fs);font-size:.82rem;font-weight:600;cursor:pointer;transition:.2s;white-space:nowrap;text-decoration:none}
...
/* ═══ INFO BOX ═══ */
.info-box{background:var(--blue-soft);border:1.5px solid rgba(26,86,219,.15);border-radius:var(--r);padding:12px 16px;display:flex;align-items:flex-start;gap:10px;margin-bottom:22px}
.info-box i{color:var(--blue);font-size:.95rem;flex-shrink:0;margin-top:1px}
.info-box p{font-family:var(--fs);font-size:.82rem;color:var(--muted);line-height:1.6}
.info-box strong{color:var(--ink);font-weight:600}

/* ═══ FORM ═══ */
.form-wrap{max-width:600px;width:100%}
...
.card-section-label{font-family:var(--fs);font-size:.6rem;font-weight:600;letter-spacing:.22em;text-transform:uppercase;color:var(--blue);margin-bottom:18px;display:flex;align-items:center;gap:8px}
.card-section-label::before{content:'';width:3px;height:12px;background:var(--blue);border-radius:2px}

.field{margin-bottom:16px}
.field:last-child{margin-bottom:0}
.field label{display:block;font-family:var(--fs);font-size:.68rem;font-weight:600;color:var(--muted);margin-bottom:6px;text-transform:uppercase;letter-spacing:.08em}
.req{color:var(--red)}
...
.field-hint{font-family:var(--fs);font-size:.7rem;color:var(--muted);margin-top:5px;display:flex;align-items:center;gap:4px}

/* ═══ FORM ACTIONS ═══ */
.form-actions{padding:16px 24px;border-top:1px solid var(--border-lt);background:var(--bg2);display:flex;gap:10px;align-items:center}
.btn-primary{
  display:inline-flex;align-items:center;gap:8px;padding:10px 22px;
  background:var(--blue);color:#fff;border:none;border-radius:var(--r);
  font-family:var(--fs);font-size:.82rem;font-weight:600;letter-spacing:.02em;
...
.btn-ghost{
  ...
  border-radius:var(--r);font-family:var(--fs);font-size:.82rem;font-weight:600;
...
@media(max-width:640px){
  ...
  .form-actions .btn-primary,.form-actions .btn-ghost{width:100%;justify-content:center}
  .page-hd .btn-back{width:100%;justify-content:center}
}
  </style>
</head>
<body>
...
<div class="content">
  ...
  <div class="page-hd">
    <div>
      ...
      <h1 class="page-title">Edit <em>Kategori</em></h1>
      <p class="page-sub">Perbarui nama dan deskripsi kategori berita.</p>
    </div>
  ...
  <div class="form-wrap">
    ...
    <div class="info-box">
      ...
      <p>Sedang mengedit kategori <strong><?= htmlspecialchars($kat['nama_kategori']) ?></strong>.
      Perubahan nama akan mempengaruhi semua artikel yang menggunakan kategori ini.</p>
    </div>
    ...
    <form method="POST">
      <div class="tcard">
        <div class="card-section">
          <div class="card-section-label">Informasi Kategori</div>

          <div class="field">
            <label>Nama Kategori <span class="req">*</span></label>
            <input type="text" name="nama_kategori" required
                   placeholder="Contoh: Teknologi, Olahraga…"
                   value="<?= htmlspecialchars($_POST['nama_kategori'] ?? $kat['nama_kategori']) ?>">
            <div class="field-hint"><i class="bi bi-info-circle"></i> Nama harus unik dan belum digunakan kategori lain.</div>
          </div>

          <div class="field">
            <label>Deskripsi <span style="color:var(--faint);font-weight:400;text-transform:none;letter-spacing:0">(opsional)</span></label>
            <textarea name="deskripsi" placeholder="Deskripsi singkat kategori ini…"><?= htmlspecialchars($_POST['deskripsi'] ?? $kat['deskripsi']) ?></textarea>
          </div>
        </div>
...
if(toggle)toggle.addEventListener('click',e=>{e.stopPropagation();sidebar.classList.contains('open')?close():open();});
