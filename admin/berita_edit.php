<?php
/**
 * berita_edit.php  –  Multi-Kategori Edition (Unified Design)
 */
...
$savedKatIds = [];
if (!empty($art['kategori_ids'])) {
    $decoded = json_decode($art['kategori_ids'], true);
    if (is_array($decoded)) $savedKatIds = array_map('intval', $decoded);
}
if (empty($savedKatIds) && !empty($art['kategori_id'])) {
    $savedKatIds = [(int)$art['kategori_id']];
}
    ...
    if (empty($judul) || empty($isi)) {
        ...
        if (!$error) {
            ...
            mysqli_query($koneksi, "UPDATE artikel
                SET judul='$judul', isi='$isi', thumbnail='$thumbVal',
                    kategori_id=$primaryKatId,
                    kategori_ids='$katIdsJson',
                    kategori_manual=1,
                    status='$status'
                WHERE id_artikel=$id");

            header("Location: kelola_berita.php?msg=edit"); exit;
        }
    }

    $savedKatIds = $kat_ids_raw ? array_map('intval', $kat_ids_raw) : $savedKatIds;
}

function kat_color_edit($nama) {
    $map = [
...
<head>
  ...
  <title>Edit Berita — LiyNews Admin</title>
  <script>
    (function(){
...
.img-current{width:100%;border-radius:var(--r);border:1px solid var(--border);margin-bottom:10px;display:block}
.img-badge{display:inline-flex;align-items:center;gap:5px;font-size:.67rem;font-weight:600;color:var(--blue);background:var(--blue-soft);border:1px solid rgba(26,86,219,.2);border-radius:99px;padding:3px 10px;margin-bottom:10px;font-family:var(--fs)}
...
<div class="content">
  <div class="page-hd">
    <div>
      ...
      <h1 class="page-title">Edit <em>Berita</em></h1>
      <p class="page-sub">Perbarui isi artikel berita — termasuk kategori (bisa lebih dari satu).</p>
    </div>
          ...
          <div style="padding:20px">
            <div class="field">
              ...
              <input type="text" name="judul" required
                     ...
                     value="<?= htmlspecialchars($_POST['judul'] ?? $art['judul']) ?>">
            </div>
            <div class="field" style="margin-bottom:0">
              <label>Isi Berita <span class="req">*</span></label>
              <textarea name="isi" required placeholder="Tulis isi berita di sini…"><?= htmlspecialchars($_POST['isi'] ?? $art['isi']) ?></textarea>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="side-stack">
            ...
            <div class="status-grid">
              <label class="status-opt">
                <input type="radio" name="status" value="publish"
                  <?= ($_POST['status']??$art['status'])==='publish'?'checked':'' ?>>
                <span class="status-label"><i class="bi bi-globe"></i> Publish</span>
              </label>
              <label class="status-opt">
                <input type="radio" name="status" value="draft"
                  <?= ($_POST['status']??$art['status'])==='draft'?'checked':'' ?>>
          ...
          <div style="padding:20px">
            ...
            <div class="kat-grid" id="katGrid">
              <?php foreach ($allKats as $k):
                $checked = in_array((int)$k['id_kategori'], $savedKatIds);
                $col     = kat_color_edit($k['nama_kategori']);
        ...
        <div class="card">
          ...
          <div style="padding:20px">
            ...
            <div class="img-badge"><i class="bi bi-image-fill"></i> Gambar saat ini</div>
            <img src="../uploads/<?= htmlspecialchars($art['thumbnail']) ?>"
                 class="img-current" alt="Thumbnail saat ini">
            <?php endif; ?>
            <label class="upload-area" for="thumbnailInput">
              <i class="bi bi-cloud-arrow-up"></i>
              <p><strong>Klik untuk unggah</strong> gambar baru</p>
